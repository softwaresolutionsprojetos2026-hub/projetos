...
$(document).ready(function() {
    ...
    setTimeout(function() { 
        $(".alerta-sistema").fadeOut("slow"); 
    }, 3000);

    // --- DINÂMICA DE BOTÕES DESABILITADOS E ALTERNAÇÃO DE TELAS ---
        $('button[data-bs-toggle="tab"]').on('click', function (e) {
            e.preventDefault();
            var targetId = $(this).attr('data-bs-target');
            
            // Força o Bootstrap a alternar a visibilidade das telas de conteúdo
            $('.tab-pane').removeClass('show active');
            $(targetId).addClass('show active');

            // Gerencia qual botão fica desabilitado
            if (targetId === '#list-p') {
                $('#btn-aba-lista').prop('disabled', true);
                $('#btn-aba-cadastro').prop('disabled', false);
            } else if (targetId === '#cad-p') {
                $('#btn-aba-cadastro').prop('disabled', true);
                $('#btn-aba-lista').prop('disabled', false);
            }
        });    
...
// --- EXCLUSÃO DE IMAGENS JÁ SALVAS VIA AJAX ---
        $(document).on("click", ".btn-deletar-salva-js", function() {
            var btn = $(this);
            var imgId = btn.data("id");
            var blocoImagem = $("#img-salva-" + imgId);

            if (confirm("Tem certeza que deseja deletar esta imagem definitivamente do sistema?")) {
                $.ajax({
                    url: "router.php",
                    type: "POST",
                    data: {
                        acao: "excluir_imagem_avulsa",
                        img_id: imgId
                    },
                    dataType: "json",
                    success: function(response) {
                        if (response.sucesso) {
                            // Some com a imagem da tela com efeito suave
                            blocoImagem.fadeOut("fast", function() {
                                $(this).remove();
                            });
                        } else {
                            alert("Erro ao deletar: " + (response.erro || "Erro desconhecido."));
                        }
                    },
                    error: function() {
